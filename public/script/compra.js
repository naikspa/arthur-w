    ...
    document.addEventListener("keypress", (event) => {
      if (event.key === "Enter") {
        ...
        let productId = e.target.parentElement.parentElement.parentElement
          ...
          .getAttribute("p-id");
        let sizeInput =
          e.target.parentElement.querySelector(".select-inp-size");
        ...
        e.target.parentElement.querySelector(".sizeInp").style.display = "none";
      }
    });
  }
});
...
const changeExpand = (e) => {
  expand = e.target;
  let expanded =
    expand.parentElement.parentElement.querySelector(".second-info");
...
carrito2.addEventListener("click", (e) => {
  if (
    e.target.classList.contains("amount-ok") ||
    e.target.classList.contains("p-delete_cart") ||
    e.target.classList.contains("size-ok")
  ) {
    location.reload();
  }
});
